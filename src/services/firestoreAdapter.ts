...
export class FirestoreAdapter implements IDatabaseService {
  ...
  private visitors: SiteVisitorLog[] = getLocalItem(STORAGE_KEYS.VISITORS, []);
  ...
  private initRealtimeListeners() {
    ...
    // 12. Real Site Visitors & Analytics
    this.subscribeCollection<SiteVisitorLog>(
      DB_COLLECTIONS.VISITORS,
      STORAGE_KEYS.VISITORS,
      [],
      (items) => { 
        // Sort latest first
        this.visitors = items.sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime()); 
      }
    );
  ...
  // --- Real Site Visitors & Analytics ---
  getVisitors(): SiteVisitorLog[] {
    return this.visitors;
  }
  async addVisitor(log: SiteVisitorLog): Promise<void> {
    this.visitors = [log, ...this.visitors.filter(v => v.id !== log.id)].slice(0, 3000);
    setLocalItem(STORAGE_KEYS.VISITORS, this.visitors);
    this.notifyUpdate(DB_COLLECTIONS.VISITORS);

    try {
      const docRef = doc(db, DB_COLLECTIONS.VISITORS, log.id);
      await setDoc(docRef, sanitizeForFirestore(log), { merge: true });
    } catch (error) {
      console.warn(`Notice saving visitor log ${log.id} to cloud Firestore:`, error);
    }
  }
  async clearVisitors(): Promise<void> {
    ...
    try {
      const colRef = collection(db, DB_COLLECTIONS.VISITORS);
      const snap = await getDocs(colRef);
      const batch = writeBatch(db);
      snap.docs.forEach(d => batch.delete(d.ref));
      await batch.commit();
    } catch (error) {
      console.warn('Notice clearing visitor logs in cloud Firestore:', error);
    }
  }
