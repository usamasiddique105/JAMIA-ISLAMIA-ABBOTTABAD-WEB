...
export const AdminDashboard: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'bookings' | 'questions' | 'fatwas' | 'results' | 'news' | 'books' | 'faculty' | 'donations' | 'settings'>('overview');
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      // Check saved passcode for usamasiddique105@gmail.com
      const savedMasterCode = localStorage.getItem('jamia_admin_usama_code');

      if (savedMasterCode && p !== savedMasterCode) {
        // Incorrect password entered
        setLoginError('پاس ورڈ درست نہیں ہے۔ براہ کرم اپنا درست پاس ورڈ درج فرمائیں۔');
        setIsAuthLoading(false);
        return;
      }
      ...
      const firebasePass = p.length < 6 ? p.padEnd(6, '0') : p;
      ...
      try {
        ...
        if (code === 'auth/user-not-found' || code === 'auth/invalid-credential') {
          try {
            const newUser = await createUserWithEmailAndPassword(auth, email, firebasePass);
            if (newUser.user) {
              setCurrentUser(newUser.user);
              authSuccess = true;
            }
          } catch (createErr: any) {
            // Attempt with padded password if needed
            try {
              const res = await signInWithEmailAndPassword(auth, email, firebasePass);
              if (res.user) {
                setCurrentUser(res.user);
                authSuccess = true;
              }
            } catch (pErr) {
              console.warn('Direct email pass failed, fallback to active token:', pErr);
            }
          }
        }
      }

      // If Firebase Auth standard provider isn't accessible, ensure anonymous auth session for Firestore
      if (!auth.currentUser) {
        try {
          ...
          console.warn('Anonymous fallback token info:', anonErr);
        }
      }

      // Store the official registered admin code
      localStorage.setItem('jamia_admin_usama_code', p);
  ...
  const handleLogout = async () => {
    ...
    sessionStorage.removeItem('jamia_admin_auth');
    localStorage.removeItem('jamia_admin_auth');
    localStorage.removeItem('jamia_admin_logged_user');
    setLoginPassword('');
  };
