    ...
    <div className="space-y-16 sm:space-y-20 lg:space-y-24 font-urdu text-stone-900 dark:text-stone-100" dir="rtl">
      ...
      {isSectionEnabled('portals') && (
        ...
        >
          ...
          <div className="absolute top-0 inset-x-0 h-[2px] sm:h-[2px] bg-gradient-to-r from-transparent via-[#D4AF37]/65 to-transparent group-hover:via-[#B88A3B] transition-all duration-500 pointer-events-none z-10" />
          <div className="absolute bottom-0 inset-x-0 h-[1px] sm:h-[1px] bg-gradient-to-r from-transparent via-[#D4AF37]/35 to-transparent group-hover:via-[#B88A3B]/80 transition-all duration-500 pointer-events-none z-10" />

          {/* Left Cream Curved Arc Container with Dark Icon */}
          <div className="relative z-10 w-11 xs:w-12 sm:w-20 md:w-24 lg:w-26 shrink-0 self-stretch bg-gradient-to-br from-[#F7ECCF] via-[#EED8A1] to-[#DFC27F] dark:from-[#243044] dark:to-[#1E283A] group-hover:from-[#F3E2B6] group-hover:to-[#D4AF37] dark:group-hover:from-[#2D3C55] dark:group-hover:to-[#243044] rounded-r-[32px] xs:rounded-r-[40px] sm:rounded-r-[60px] md:rounded-r-[72px] lg:rounded-r-[80px] flex items-center justify-center border-r border-[#DEC998] sm:border-[#CBAA67] dark:border-amber-700/50 group-hover:border-[#B88A3B]/80 shadow-inner transition-all duration-500 ease-out">
            {/* Scholar Pen / Ornate Calligraphy Emblem SVG */}
            <svg viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-5.5 h-5.5 xs:w-6 xs:h-6 sm:w-10 sm:h-10 md:w-11 md:h-11 lg:w-12 lg:h-12 text-[#241F1A] dark:text-amber-300 drop-shadow-[0_1px_1px_rgba(255,255,255,0.4)] transition-transform duration-500 group-hover:scale-105">
          ...
          {/* Right Text Block */}
          <div className="relative z-10 grow pl-0.5 xs:pl-1 pr-1 xs:pr-2 sm:pr-3 md:pr-4 lg:pr-5 py-0.5 sm:py-1 space-y-0 min-w-0 flex flex-col justify-center" dir={language === 'en' ? 'ltr' : 'rtl'}>
            <h3 className={`text-center text-[12.5px] xs:text-[14px] sm:text-base md:text-[17px] lg:text-[19px] font-bold text-[#1A1612] dark:text-white group-hover:text-[#5C4632] dark:group-hover:text-amber-300 transition-colors whitespace-nowrap ${language === 'en' ? 'font-sans leading-tight' : 'font-urdu leading-[2] pt-0.5'}`}>
              {language === 'ar' ? 'مؤسس الجامعة' : language === 'en' ? 'Founder & Elders' : 'بانیِ جامعہ'}
            </h3>
            <p className={`text-right w-full text-[9.5px] xs:text-[11px] sm:text-xs md:text-[13px] lg:text-[14px] text-[#363028] dark:text-slate-300 whitespace-nowrap overflow-visible font-medium ${language === 'en' ? 'font-sans leading-snug' : 'font-urdu leading-[1.7]'}`}>
        ...
        >
          ...
          <div className="relative z-10 grow pl-0.5 xs:pl-1 pr-0 xs:pr-1 sm:pr-2.5 md:pr-4 py-0.5 sm:py-1 space-y-0 min-w-0 flex flex-col justify-center" dir={language === 'en' ? 'ltr' : 'rtl'}>
            <h3 className={`text-center text-[12.5px] xs:text-[14px] sm:text-base md:text-[17px] lg:text-[19px] font-bold text-[#1A1612] dark:text-white group-hover:text-[#5C4632] dark:group-hover:text-amber-300 transition-colors whitespace-nowrap ${language === 'en' ? 'font-sans leading-tight' : 'font-urdu leading-[2] pt-0.5'}`}>
              {language === 'ar' ? 'دار الإفتاء' : language === 'en' ? 'Darul Ifta' : 'دار الافتاء'}
            </h3>
            <p className={`text-right w-full pr-0 text-[9.5px] xs:text-[11px] sm:text-xs md:text-[13px] lg:text-[14px] text-[#363028] dark:text-slate-300 whitespace-nowrap overflow-visible font-medium ${language === 'en' ? 'font-sans leading-snug' : 'font-urdu leading-[1.7]'}`}>
        ...
        >
          ...
          <div className="p-4 sm:p-5 space-y-4 flex flex-col justify-between grow">
            <div className="space-y-2.5 text-xs font-semibold">
              <div 
                onClick={() => setCurrentTab('online-quran')}
              ...
              >
                <div className="min-w-0">
                  <div className="text-stone-900 dark:text-stone-100 font-bold text-sm leading-snug group-hover:text-[#B88A3B] transition-colors">
                    {language === 'ar' ? 'تحفيظ القرآن الكريم والناظرة بالتجويد' : language === 'en' ? 'Online Quran Nazra & Tajweed' : 'آن لائن ناظرہ و تجوید القرآن'}
                  </div>
                  <div className="text-stone-600 dark:text-stone-400 font-normal text-xs mt-0.5">
                    {language === 'ar' ? 'تعليم القرآن الكريم بأحكام التجويد ومخارج الحروف' : language === 'en' ? 'Learn Quran with correct Tajweed rules' : 'مستند قراء و اساتذہ کے زیرِ نگرانی درست مخارج کے ساتھ'}
                  </div>
                </div>
                <span className="bg-[#B88A3B] text-white px-2 py-0.5 rounded text-[11px] font-semibold shrink-0">
                  {language === 'ar' ? 'تسجيل' : language === 'en' ? 'Admission' : 'رجسٹریشن'}
                </span>
              </div>

              <div 
                onClick={() => setCurrentTab('online-dars-nizami')}
                className="p-3 rounded-md bg-[#F8F6F0] dark:bg-slate-800 border border-stone-200 dark:border-slate-700 hover:border-[#B88A3B] transition-all cursor-pointer flex items-center justify-between gap-2 group"
              >
                <div className="min-w-0">
                  <div className="text-stone-900 dark:text-stone-100 font-bold text-sm leading-snug group-hover:text-[#B88A3B] transition-colors">
                    {language === 'ar' ? 'الدرس النظامي للرجال والنساء (عالم كورس)' : language === 'en' ? 'Online Dars-e-Nizami (Alim Course)' : 'آن لائن درسِ نظامی (عالم و عالمہ کورس)'}
                  </div>
                  <div className="text-stone-600 dark:text-stone-400 font-normal text-xs mt-0.5">
                    {language === 'ar' ? 'منهاج وفاق المدارس العربية لعلوم الشريعة' : language === 'en' ? 'Wifaqul Madaris approved traditional curriculum' : 'وفاق المدارس العربیہ کا مستند نصاب و اسناد'}
                  </div>
                </div>
                <span className="bg-[#5C4632] text-white px-2 py-0.5 rounded text-[11px] font-semibold shrink-0">
                  {language === 'ar' ? 'جديد' : language === 'en' ? 'Regular' : 'داخلہ جاری'}
                </span>
              </div>
              ...
              >
                <div className="min-w-0">
                  <div className="text-stone-900 dark:text-stone-100 font-bold text-sm leading-snug group-hover:text-[#B88A3B] transition-colors">
                    {language === 'ar' ? 'حفظ القرآن الكريم عن بعد' : language === 'en' ? 'Online Hifz-ul-Quran' : 'آن لائن حفظِ قرآن کریم'}
                  </div>
                  <div className="text-stone-600 dark:text-stone-400 font-normal text-xs mt-0.5">
                    {language === 'ar' ? 'نظام يومي للسبق والسبقي والمنزل' : language === 'en' ? 'Daily Sabaq, Sabqi and Manzil routine' : 'روزانہ سبق، سبقی و منزل اور انفرادی توجہ کا منظم نظام'}
                  </div>
                </div>
                <span className="bg-[#5C4632] text-white px-2 py-0.5 rounded text-[11px] font-semibold shrink-0">
                  {language === 'ar' ? 'انفرادي' : language === 'en' ? '1-on-1' : 'انفرادی'}
                </span>
              </div>

              <div 
                onClick={() => setCurrentTab('online-classes')}
                className="p-3 rounded-md bg-[#F8F6F0] dark:bg-slate-800 border border-stone-200 dark:border-slate-700 hover:border-[#B88A3B] transition-all cursor-pointer flex items-center justify-between gap-2 group"
              >
                <div className="min-w-0">
                  <div className="text-stone-900 dark:text-stone-100 font-bold text-sm leading-snug group-hover:text-[#B88A3B] transition-colors">
                    {language === 'ar' ? 'دورات ترجمة القرآن واللغة العربية' : language === 'en' ? 'Quran Translation & Arabic Course' : 'ترجمہ قرآن، صرف و نحو و عربی کورس'}
                  </div>
                  <div className="text-stone-600 dark:text-stone-400 font-normal text-xs mt-0.5">
                    {language === 'ar' ? 'فهم القرآن الكريم وقواعد اللغة العربية للمبتدئين' : language === 'en' ? 'Understanding Quran & Arabic for beginners' : 'بنیادی عربی گرامر، نماز و ضروری احکام کے مختصر کورسز'}
                  </div>
                </div>
                <span className="bg-[#5C4632] text-white px-2 py-0.5 rounded text-[11px] font-semibold shrink-0">
                  {language === 'ar' ? 'دورات' : language === 'en' ? 'Courses' : 'کورسز'}
                </span>
              </div>
            </div>

            <button 
              onClick={() => setCurrentTab('online-services')}
            ...
            >
              {language === 'ar' ? 'تفاصيل الدروس والتسجيل ◀' : language === 'en' ? 'View Online Classes & Admission ◀' : 'آن لائن کلاسز و داخلہ دیکھیے ◀'}
            </button>
          </div>
        </motion.div>
        )}
