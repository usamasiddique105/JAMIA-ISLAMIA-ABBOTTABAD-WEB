...
export const OnlineServicesView: React.FC<OnlineServicesViewProps> = ({
  ...
  const primaryTabsList = [
    ...
    { 
      id: 'online-taawun', 
      label: isAr ? 'طريقة التعاون' : isEn ? 'Donations & Support' : 'طریقہ تعاون', 
      icon: Heart,
      badge: isAr ? 'زكاة وتبرعات' : isEn ? 'Zakat & Donations' : 'زکوٰۃ و عطیات',
      desc: isAr ? 'الحسابات البنكية للزكاة والصدقات' : isEn ? 'Bank accounts for Zakat & Sadaqah' : 'زکوٰۃ، صدقات اور عطیات کے بنک اکاؤنٹس'
    },
    { 
      id: 'online-admission', 
      label: isAr ? 'استمارة القبول الإلكتروني' : isEn ? 'Online Admission Form' : 'آن لائن داخلہ فارم', 
      icon: Send,
      badge: isAr ? 'مفتوح' : isEn ? 'Open' : 'اوپن',
      desc: isAr ? 'تسجيل الطلاب بسهولة عبر الإنترنت' : isEn ? 'Easy online student registration' : 'طالب علم کی آسان اور آن لائن رجسٹریشن'
    },
    { 
      id: 'online-trial', 
          ...
          {primaryTabsList.map((tab) => {
            ...
            return (
              <button
                ...
                className={`p-3.5 rounded-xl transition-all ${isEn ? 'text-left' : 'text-right'} border cursor-pointer relative flex flex-col justify-between group ${
                  isSelected
              ...
              >
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className={`text-[10px] ${fontClass} font-bold px-2 py-0.5 rounded-full border ${
                      isSelected 
                  ...
                  <h3 className={`text-base font-bold ${fontClass} leading-tight ${isSelected ? 'text-[#B88A3B]' : 'text-stone-900 dark:text-stone-100'}`}>
                    {tab.label}
                  </h3>
                  <p className={`text-xs mt-1 ${fontClass} line-clamp-1 ${isSelected ? 'text-stone-200' : 'text-stone-500 dark:text-stone-400'}`}>
                    {tab.desc}
                  </p>
                </div>

                <div className={`mt-3 ${isEn ? 'text-right' : 'text-left'}`}>
                  <span className={`text-[11px] ${fontClass} font-bold inline-flex items-center gap-1 ${isSelected ? 'text-[#B88A3B]' : 'text-[#5C4632] dark:text-amber-300'}`}>
                    <span>{isAr ? 'عرض التفاصيل' : isEn ? 'View Details' : 'تفصیلات دیکھیں'}</span>
                    <span>{isEn ? '→' : '←'}</span>
                  </span>
                </div>
              </button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {(darsSubCourse === 'all' || darsSubCourse === 'online-arabic') && (
                <div className="bg-[#F8F4EC] dark:bg-slate-900 border-2 border-[#B88A3B]/60 rounded-xl p-5 space-y-3">
                  ...
                  <button onClick={() => handlePrimaryTabChange('online-admission')} className={`w-full mt-2 py-1.5 bg-[#5C4632] text-white text-xs ${fontClass} font-bold rounded-lg hover:bg-[#433324] cursor-pointer`}>
